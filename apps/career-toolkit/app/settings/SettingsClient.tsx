      ...
      <div style={{ maxWidth: "600px" }}>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <span className={`badge ${isPro ? "badge-success" : "badge-default"}`}>{isPro ? "Pro" : "Free"}</span>
          </div>

          {isPro ? (
            <p style={{ fontSize: "0.875rem", color: "var(--muted-foreground)" }}>
              You have unlimited career toolkit actions. Thank you for being a Pro member.
            </p>
          ) : (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                <span style={{ fontSize: "0.875rem", color: "var(--muted-foreground)" }}>
                  {usage.used} of {usage.limit} actions used this month
                </span>
              </div>
              <div style={{ height: "4px", background: "var(--border)", marginBottom: "1.5rem" }}>
                <div style={{ height: "100%", width: `${Math.min((usage.used / usage.limit) * 100, 100)}%`, background: usage.used >= usage.limit ? "#FF3333" : "#0066FF", transition: "width 0.3s" }} />
              </div>
              <div style={{ border: "1px solid #0066FF", padding: "1.5rem", background: "#0066FF08" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
                  <div>
                    ...
                    <div className="input-label" style={{ marginTop: "0.25rem" }}>Unlimited career actions</div>
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1.5rem" }}>
                  {["Unlimited resume optimizations", "Unlimited cover letters", "Unlimited job match analysis", "Priority support"].map((f) => (
                ...
                <Button onClick={handleUpgrade} loading={upgrading} fullWidth>Upgrade to Pro →</Button>
              </div>
            </div>
          )}
        </div>
